...
export const getFacilities = createAsyncThunk(
    ...
    async ({ body,token}, thunkAPI) => {
      try {
        const response = await api.post(`reactapi/`,'fetchFacilities',body,token);
...
extraReducers: {
    [getFacilities.fulfilled]: (state, {payload} ) => {
    state.isFetching    = false;
        state.facilitiesDetails   = payload.apiRet;
    },
    [getFacilities.rejected]: (state,  {payload} ) => {
    state.isFetching    = false;
    },
...
export const { ADD_STATE,UPDATE_STATE,FETCH_STATE,REMOVE_STATE} = FacilitiesSlice.actions;
