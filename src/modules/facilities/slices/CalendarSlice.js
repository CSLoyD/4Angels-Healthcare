...
export const getCalendarSchedules = createAsyncThunk(
    'facilities/getCalendarSchedules',
    async ({ body,token,}, thunkAPI) => {
      try {
        const response = await api.post(`reactapi/`,'fetchShiftsforCalendarView/',body,token);
...
const initialState = {
isFetching : true,
calendarSchedDetails    : [],
}


const CalendarSchedSlice = createSlice({
name: 'calendarSched',
...
extraReducers: {
    [getCalendarSchedules.fulfilled]: (state, {payload} ) => {
    state.isFetching    = false;
        state.calendarSchedDetails   = payload.apiRet;
    },
    [getCalendarSchedules.rejected]: (state,  {payload} ) => {
    state.isFetching    = false;
    },
    [getCalendarSchedules.pending]: (state) => {
...
export const { ADD_STATE,UPDATE_STATE,FETCH_STATE,REMOVE_STATE} = CalendarSchedSlice.actions;
export default CalendarSchedSlice.reducer;  