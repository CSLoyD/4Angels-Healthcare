...
export const getShifts = createAsyncThunk(
    ...
    async ({ body,token}, thunkAPI) => {
      try {
        const response = await api.post(`reactapi/`,'fetchShifts',body,token);
...
extraReducers: {
    ...
    state.isFetching    = false;
        state.shiftsDetails   = payload.apiRet;
    },
...
export const { ADD_STATE,UPDATE_STATE,FETCH_STATE,REMOVE_STATE} = ShiftsSlice.actions;
